...
func (q *Queries) GetNextFeedsToFetch(ctx context.Context, numberOfResults int32) ([]Feed, error) {
	rows, err := q.db.QueryContext(ctx, getNextFeedsToFetch, numberOfResults)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) MarkFeedFetched(ctx context.Context, arg MarkFeedFetchedParams) (Feed, error) {
	row := q.db.QueryRowContext(ctx, markFeedFetched, arg.NewTime, arg.FeedID)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
